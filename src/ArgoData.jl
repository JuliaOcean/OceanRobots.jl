...
"""
    MITprof_loop(pth::String="profiles/")

Standard Depth Argo Data Collection -- see `?MITprof_read` for detail.
"""
function MITprof_loop(pth::String="profiles/")
    ...
    lo=[]; la=[]; ye=[];
    for f=1:length(λ)
        (tmplo,tmpla,tmpye)=MITprof_read(pth*λ[f])
