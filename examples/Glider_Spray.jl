...
begin
	...
	p=Mkie.lines!(a_uv,gdf[ID].u[:])
	p=Mkie.lines!(a_uv,gdf[ID].v[:])
	p=Mkie.lines!(a_uv,sqrt.(gdf[ID].u[:].^2 + gdf[ID].v[:].^2))

	a2=Mkie.Axis(f[2,1],title="Temperature (degree C -- 10,100,500m depth)")

	Mkie.lines!(a2,gdf[ID].T10[:])	
	Mkie.lines!(a2,gdf[ID].T100[:])
	Mkie.lines!(a2,gdf[ID].T500[:])

	a3=Mkie.Axis(f[2,2],title="Salinity (psu -- 10,100,500m depth)")

	Mkie.lines!(a3,gdf[ID].S10[:],label="10m")	
	Mkie.lines!(a3,gdf[ID].S100[:],label="100m")
	Mkie.lines!(a3,gdf[ID].S500[:],label="500m")
...
julia_version = "1.8.1"
manifest_format = "2.0"
project_hash = "89604ff259d846b5269fbf48d9dc709086638271"
