...
function animate_positions(F,B; framerate = 20, time_axis_method = 0, time_stamps=missing)
    if time_axis_method==1
        ...
        ii = @lift(findall( time_stamps.== $time)[1])
        iim1 = @lift( max($ii-1,1) )
        iim2 = @lift( max($ii-2,1) )
        iim3 = @lift( max($ii-3,1) )
        iim4 = @lift( max($ii-4,1) )
...
#    c = @lift( [B.df[1].col[$ii] ;fill(0,nmax-length($ii))])
    gdf = DataFrames.groupby(B.df[1], :d)
    x = @lift( vcat(gdf[$ii].x,gdf[$iim1].x,gdf[$iim2].x,gdf[$iim3].x,gdf[$iim4].x) )
    y = @lift( vcat(gdf[$ii].y,gdf[$iim1].y,gdf[$iim2].y,gdf[$iim3].y,gdf[$iim4].y) )
    c = @lift( vcat(gdf[$ii].col,gdf[$iim1].col,gdf[$iim2].col,gdf[$iim3].col,gdf[$iim4].col) )
